...
function () {
  function RenderGift(opts) {
    ...
    self.loadModel(self.modelName).then(function (object) {
      ...
      self.getBoxGrid(self.model, 10, 2); // end of experiment;
  ...
  _createClass(RenderGift, [{
    ...
    value: function getBoxGrid(model, amount, separationMultiplier) {
      ...
      for (var i = 0; i < amount; i++) {
        ...
        obj.position.y = 0; // obj.position.z = i * separationMultiplier;

        obj.scale.set(self.scale, self.scale, self.scale);
        obj.rotation.set(Math.PI / 8, Math.PI / 4, 0);
        obj.castShadow = true;
        self.scene.add(obj); // for (var j = 1; j < amount; j++) {
        //   var obj = model.clone(true);
        //   obj.scale.set(self.scale, self.scale, self.scale);
        //   obj.rotation.set(Math.PI / 8, Math.PI / 4, 0);
        //   obj.castShadow = true;
        //   obj.position.x = i * separationMultiplier;
        //   // obj.position.y = 0;
        //   obj.position.z = j * separationMultiplier * 2;
        //   self.scene.add(obj);
        // }
    ...
    value: function getScene() {
      ...
      thisSpotlight.add(spotLight.position, 'z', -100, 100); // const plane = self.getPlane(1000, 1000, 0x4c95eb);
      // plane.name = "plane";
...
//# sourceMappingURL=app.b4e5964d688094081879.hot-update.js.map